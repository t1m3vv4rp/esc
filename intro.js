...
const F_LATER = [
  ...
  "오늘은 대박이 터질 날입니다. 이왕이면 로또 당첨되는 게 좋겠네요. 좋은 하루 되세요!",
...
function submitFortune(){
  ...
  if(!firstFortuneDone){
    ...
    // 이후 재검색
    let txt;
    if(name===playerFull){ txt = (name==="김유진")?F_YUJIN:F_MAIN; }
...
function endIntro(){
  ...
  const isSpecial = ["박가연","이주은","김아량"].includes(playerFull);
  const mono = isSpecial
    ? "오늘은 처음으로 3층 A해보는 날이니까… 운영을 하러 가볼까."
    : "오늘은 3층 A니까… 운영을 하러 가야지.";
  ...
  setTimeout(()=>{
    ...
    m.style.cssText="position:fixed;z-index:310;left:50%;top:50%;transform:translate(-50%,-50%);background:#f4f4f2;color:#111;padding:16px 26px;border-radius:10px;font-size:15px;box-shadow:0 4px 0 rgba(0,0,0,0.5);max-width:80%;text-align:center;";
    m.textContent = mono;
    ...
    setTimeout(()=>{
      ...
      dis.style.opacity=1;
      setTimeout(()=>{
        // 다음 단계(책상 수색)는 아직 미구현 → 미니게임으로 임시 연결
